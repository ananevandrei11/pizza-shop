  ...
  return (
    <figure className={cn(`relative flex items-center justify-center pb-[100%] h-0`, className)}>
      <Image
        src={src}
        alt={alt || 'Pizza image'}
        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 block w-${sizeClassName} w-${sizeClassName} rounded-full object-cover object-center transition-all z-9 duration-300`}
        width={sizeImage}
        height={sizeImage}
      />
